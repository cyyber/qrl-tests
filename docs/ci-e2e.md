...
is built and pushed. Every source repository resolves to its upstream tip:
the node and Clef images build from the resolved go-qrl revision
(`Dockerfile` and `Dockerfile.alltools`), the qrysm beacon and validator
images from the resolved qrysm revision, and the genesis generator from the
resolved generator revision plus the qrysm one (it embeds qrysm tooling).
